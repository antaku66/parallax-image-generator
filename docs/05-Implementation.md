...
| PR4 | segmentation interface、segmentation 推論、mask 生成 | 一部（MODNet〔Apache-2.0, 任意配置〕の推論基盤 + 信頼度ゲート + 深度解像度での最前面マット融合まで実装。適用状況は `metadata.segmentation` / PerfBadge で計測できる。テクスチャ解像度での融合〔髪などの細部回復〕と単層→2層昇格は、実画像での導入効果を確認してから判断） |
| PR5 | depth histogram によるレイヤー分割、Layered Depth Image 生成 | 実装済み（再帰的 Otsu による 1〜maxLayers 層の自動決定。深度のみで分割し、segmentation による境界補正は PR4 の導入判断後。実装ガイド §13 の `LAYER_DEFAULTS` とは意図的な差異あり: `minLayerCount: 3` は単峰分布への強制分割が「面が裂ける」破綻を生むため不採用〔ゲートによる自動決定を維持〕、`maxLayerCount: 6` はメモリ・処理時間の観点で tier 別 3/4 に縮小、`minRegionAreaRatio`〔連結成分の面積棄却〕は segmentation とセットの概念のためスラブ質量ゲートで代替） |
...
segmentation は独立した処理として後から追加できる構成にしてある（追加しても既存の描画・キャッシュ経路は変わらない。`LayeredRenderer` は任意レイヤー数に対応済み）。パイプラインの処理内容を変えた場合（`PIPELINE_DEFAULTS` / `IMAGE_LIMITS` の値変更を含む）は `constants/versions.ts` の `PROCESSING_VERSION` を上げてキャッシュを無効化する。キャッシュキーには tier（mobile/desktop）も含まれ、tier 依存の解像度差で別品質の資産を誤ヒットしない。
...
int8 量子化起因の低周波ノイズ（平面の波打ち・帯状ノイズ）が気になる場合は、同リポジトリの fp16 ONNX（`onnx/model_fp16.onnx`, 約190MB）を `public/models/depth-anything-v2-base-fp16.onnx` に配置し、`constants/models.ts` の `DEFAULT_MODEL` を `depth-anything-v2-base-fp16` に変更する。fp16 で消えれば量子化起因、残れば Base の容量限界のため Large を検討する。
...
前景マッティング（segmentation）を有効にする場合は、[Hugging Face `Xenova/modnet`](https://huggingface.co/Xenova/modnet) の量子化 ONNX（`onnx/model_quantized.onnx`, 約6.6MB）を `public/models/modnet.onnx` として配置する。未配置なら seg なしの深度のみマットで動作する（エラーにはならない）。品質不足なら同リポジトリの fp16 版（約13MB）へ差し替え、`constants/models.ts` の `SEG_MODELS.modnet.version` を変えてキャッシュを無効化する。
...
- `splitDepthLayers` / `maskOps` / `pushPullInpaint` / `discontinuityNearMask`（レイヤー生成コア）
- `segmentationGate` / `fuseMatte`（seg 統合の判定と融合）
