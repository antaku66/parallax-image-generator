...
flowchart TD
    ...
    E --> G["レイヤー生成（分割判定 → 単層 or 多層 LDI + 各層インペイント）"]
...
- **分割判定**: `splitDepthLayers` が**再帰的 Otsu**（区間限定 Otsu の貪欲 best-first 二分割）でレイヤー境界しきい値 t_1 < … < t_{K-1} と、各しきい値の累積 near 側ソフトマスク M_k を求める。分割候補は次のゲートをすべて通過した場合のみ採択される:
  ...
  採択ゼロ（深度が連続的な風景等）は分割せず、不連続カリング付きの**連続メッシュ 1 枚（単層）**へフォールバックする（「一枚の面が裂ける」破綻を防ぐ）。
- **N 層の統一スキーム**: レイヤー k = 「t_{k+1} より手前（M_{k+1}）を穴としてインペイントした画像」を M_k でマットしたもの。
  ...
  - **中間層**: 自スラブの切り抜きに加え、さらに手前の領域を自スラブの延長色で穴埋めする（露出時に奥の色でなく自スラブの色が見える）。1 回の push-pull を穴埋めと色デコンタミネーションで共用する。テクスチャ長辺は `midTextureSide`（中景は解像度感度が低い）。
  - **最前面**: 被写体の切り抜き（穴なし）。
- **マットのエッジ整合アップサンプリング**: 深度解像度のマスクはテクスチャ解像度へ双線形拡大後、輝度ガイドの guided filter（`upsampleMatte`）で実シルエットへ吸着させる。
